...
python -m tools.v5_recheck <policy.pt> <pools_v2.pkl> <vocab.npz> <out.json> [margins=0.25] [procs=4]
For every margin: win rate / money difference vs v4b on the same games (paired SE), per category, head-to-head.
"""
import json
import sys
...
def main():
    ...
    margins = [float(x) for x in (sys.argv[5] if len(sys.argv) > 5 else "0.25").split(",")]
