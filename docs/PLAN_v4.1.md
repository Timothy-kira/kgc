...
## Engram 训练结果与下一步（09-26 01:40）
- CSA2 + Engram（从 pre.pt 微调 7 小时，2×T4）验证集：对手事件准确率 81.1%、交叉熵 0.49；原策略单位动作 68.5%、市场 71.9%。
- 消融：关闭 Engram 的门后对手事件准确率 84.6% → 74.9%、交叉熵 0.42 → 0.68，策略指标不变：记忆承载了主干没有的对手信息。
- 同分段对手高度集中：2100–2700 分段中最常见的程序占 17.5%（205 支队伍），前 15 个程序覆盖 55.7%。
- 下一步（H5 + H6）：对每个常见程序单独找最优路线/开关（`tools/program_routes.py`，开环重放其 replay），半数对局选、半数检验；稳定有效的应对写成"对手程序 → 应对"表，由 Engram 在第 2 步/前几天识别对手程序后查表执行。

## 天梯对局分析与开关复核（09-26 03:00）
- 官方 CLI 下载我们的天梯对局（`tools/fetch_our_games.py`）：
  - v4a（= cha22）130 局：55 胜 22 平 53 负，资金差中位数仅 483；平局 17% 全是 cha22 克隆镜像；
  - v4b（+clamp_sells）43 局：35 胜 1 平 7 负（刚上榜、对手偏弱，暂不可比），平局几乎消失，镜像打破有效。
- 同分段胜负差距极小，常见输局 −800 ~ −2100。
- 常见程序的"程序 → 路线"表：对前 6 个程序（开环）cha22 已赢 93–100%，交叉验证下任何固定路线都不优于 cha22 按商店的选择 → 放弃。
- v4b 上再加开关（300 局配对）：room_guard +28±7 资金但胜率 83.0% vs 83.3%；budget_guard +42±327 → 都不采用，维持 v4b。

## DeepSeek 如何训练 Engram（论文 arXiv 2601.07372 + 官方仓库）与我们的对照
| 项目 | DeepSeek | 我们 |
|---|---|---|
| 表的优化器 | Adam，学习率 ×5，无权重衰减；主干用 Muon | AdamW 分组：表 ×5（`KGC_ENGRAM_LR_MULT`，原为 ×10）、无权重衰减；主干 AdamW |
| 位置 | 论文第 2、15 层；V4.1-Flash 第 1、14 层（共 40 层） | 第 1、3 层（共 6 层） |
| N-gram / 头 / 维度 | 论文 3 阶、8 头、1280 维；Flash 4 阶、8 头、256 维 | 4 阶、4 头、32 维 |
| 短卷积 | 论文：核 4、膨胀=最高阶、SiLU、零初始化保持恒等；Flash 推理代码中无 | 无（跟 Flash） |
| 门控 | 带符号开方 sigmoid；演示用两组 RMSNorm，Flash 用 q·k 权重乘积 | 同 Flash |
| 训练 | 与主干一起从头训练 2620 亿 token | 插入预训练 CSA2，value 零初始化（与论文零初始化卷积同理：起步即恒等） |
| 参数分配 | U 形规律：稀疏参数 20–25% 给 Engram 最优（语言模型） | Engram 约 2550 万，占 83%；用途是记忆高度重复的对手程序，消融显示有效（74.9%→84.6%），重训时可扫表大小验证 |
- 官方仓库只有演示代码 `engram_demo_v1.py`（无训练脚本），细节以论文为准。
