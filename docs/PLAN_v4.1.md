...
## H5（路线）结论：09-25 夜间实验后停止
- 上帝视角上限很大（每局 +1.39 万，16.6/41 条路线优于 cha22 查表），但**事先不可利用**：
  - Engram 路由表（533 局）：留出集收益为负，任何换路线阈值都一样；
  - 最简单的查表基线（按商店组合 + t 检验）：最多约 +400/局，噪声范围内；
  - 第 144 步用 cha22 替身做现场推演（55 局）：同分段对手 +944、联赛 +1,810，但顶尖队伍 −12,909，且第 144 步的资金比、地块数区分不出顶尖对手。
- 结论：路线优劣主要由该局的具体走势决定，H5 放弃；Engram 对手记忆改用于其他插入点。
- 顺带修复：`route_data` 同类别相邻 7 个 job 共用同一局 replay（重复对局），已改为每个 job 取不同对局。
- 下一个插入点：cha22 自己关掉的 5 个反应层开关（`tools/settings_sweep.py`，配对设计，对手按天梯分布）。

## 开关插入点结论与 v4b（09-26 凌晨）
- 5 个关掉的反应层对资金影响都在 ±20 以内（配对 120 局），多数被 cha22 的补丁覆盖。
- 但 `clamp_sells` 能稳定打破 cha22 镜像：对原版 cha22 80 局 60 胜 20 负 0 平；300 局配对复核总胜率 82.7% vs 80.3%，对同分段、顶尖对手胜率完全不变，资金 −22±8。
- 天梯上 cha22 克隆约占 2200–2600 分段对局的 6.1%（最近 4000 局，75 支队伍），镜像原本只能打平。
- **v4b = v4a + `clamp_sells`**，已提交；打包后对 cha22 新种子 28 胜 2 平 10 负，延迟均值 1.8 ms、最大 52 ms。
