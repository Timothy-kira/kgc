"""Post-training: token-level GRPO for the decoder-only policy (MiMo-V2.6 / CodeMidas style).
...
from agent.action_tokens import Grammar
...
from rl.token_rollout import WorkerPool, run_games
...
def grammar_masks(tr, V):
    """Legal-token masks for every predicted action token, in collate target order."""
    n_hands = (tr["units"][:, :, 34] > 0.5).sum(1) - 1
    off = tr["act_off"]
    rows = []
    for t in range(len(off) - 1):
        toks = tr["act"][off[t]:off[t + 1]]
        g = Grammar(max_hands=int(n_hands[t]), allow_base=False)
...
def token_logprobs(net, trajs, device, dtype):
    ...
    logits, value, _, aux = net.forward_train(b["ids"], obs_feats(b, dtype), b["otype"], b["tgt_pos"], b["tgt_ids"],
                                              b["val_pos"])
    masks = torch.from_numpy(np.concatenate([grammar_masks(t, logits.size(-1)) for t in trajs])).to(device)
    logits = logits.float().masked_fill(~masks, -1e9)
    lp = torch.log_softmax(logits, -1).gather(1, b["tgt_ids"][:, None]).squeeze(1)
    # trajectory id of every target token
    tid = b["tgt_pos"][:, 0]
    return lp, tid, value, b, aux
    ...
    for it in range(a.iters):
        ...
        for ep in range(a.epochs):
            ...
            for s in range(0, len(use), a.micro):
                ...
                old = torch.tensor(np.concatenate([np.concatenate([np.array(x) for x in t["lp"]]) for t in sub]),
